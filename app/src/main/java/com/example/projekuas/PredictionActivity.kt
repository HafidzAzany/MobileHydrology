...
class PredictionActivity : AppCompatActivity() {
    ...
    private fun setupSpinners() {
        ...
        findViewById<Spinner>(R.id.spHealth).adapter =
            spinnerAdapterSelectedWhiteDropdownBlack(
                arrayOf("Sehat", "Sedang Sakit/Demam")
            )

        findViewById<Spinner>(R.id.spHeatStress).adapter =
            spinnerAdapterSelectedWhiteDropdownBlack(
                arrayOf("Cuaca Normal", "Sangat Panas/Terik")
            )
        ...
        findViewById<Spinner>(R.id.spHydrationRisk).adapter =
            spinnerAdapterSelectedWhiteDropdownBlack(
                arrayOf("Jarang Haus (Rendah)", "Mudah Haus (Tinggi)")
            )
    }
    ...
    private fun doPredict() {
        ...
        val tensor = OnnxTensor.createTensor(
            env,
            java.nio.FloatBuffer.wrap(inputs),
            longArrayOf(1, 9)
        )
        ...
        findViewById<TextView>(R.id.tvResult).text =
            "Hasil: Tubuh Anda membutuhkan sekitar %.2f Liter air hari ini."
                .format(prediction)
    }
}
